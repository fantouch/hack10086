...
/**
 * 修改自FinalHttp源码,增加可配置Header[]的下载方法
 * 
 * @author Fantouch
 */
public class MyFinalHttp {
  ...
  //------------------put 请求-----------------------
    ...
    //---------------------下载---------------------------------------
    public HttpHandler<File> download(String url, String target, Header[] headers,
            AjaxCallBack<File> callback) {
        return download(url, null, headers, target, false, callback);
    }
    

    public HttpHandler<File> download(String url,String target,boolean isResume,AjaxCallBack<File> callback){
        return download(url, null, null, target, isResume, callback);
    }
    
    public HttpHandler<File> download( String url,AjaxParams params, String target, AjaxCallBack<File> callback) {
        return download(url, params, null, target, false, callback);
    }

    
    public HttpHandler<File> download(String url, AjaxParams params, Header[] headers,
